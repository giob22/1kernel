...
// ritorna se ci sono richieste eseguite dal device 
bool virtq_is_busy(struct virtio_virtq *vq);
// read/write from/to virtio-blk device
void read_write_disk(void *buff, unsigned sector, int is_write);
...
// implementazione del context switch per la gestione delle interruzioni
...
void boot(void){
	__asm__ __volatile__(
		...
		:
		: [stack_top] "r" (__stack_top) // passa l'indirizzo dello stack top come %[stack_top]
	);
...
void putchar(char ch){
	// il valore 1 in eid identifica il servizio di OpenSBI che vogliamo sia eseguito.
	// consiste nell'analogo dei system call number
...
void read_write_disk(void *buf, unsigned sector, int is_write){
	...
	// costruiamo la richiesta in accordo con le specifiche di virtio-blk
	blk_req->sector = sector;
	blk_req->type = is_write ? VIRTIO_BLK_T_OUT : VIRTIO_BLK_T_IN;
	if (is_write)
		memcpy(blk_req->data, buf, SECTOR_SIZE);

	// costruiamo i descrittori della virtqueue (utilizziamo 3 descrittori)
	...
	vq->descs[1].addr = blk_req_paddr + offserof(struct virtio_blk_req, data);
	vq->descs[1].len = SECTOR_SIZE;
	vq->descs[1].flags = VIRTQ_DESC_F_NEXT | (is_write ? 0 : VIRTQ_DESC_F_WRITE);
	vq->descs[1].next = 2;

	vq->descs[2].addr = blk_req_paddr + offserof(struct virtio_blk_req, status);
	vq->descs[2].len = sizeof(uint8_t);
	vq->descs[2].flags = VIRTQ_DESC_F_WRITE;

	// notifichiamo il device che c'è una nuova richiesta
	virtq_kick(vq, 0);
	

	// aspettiamo finché il device non finisce l'elaborazione
	// POLLING (sarebbe da evitare)
	while (virtq_is_busy(vq))
		;
	
	// virtio-blk: if a none zero value is returned, it's an error
	if (blk_req->status != 0) {
		printf("virtio: warn: failed to read/write sector=%d status=%d\n", sector, blk_req->status);
		return;
	}

	// for read operation, copy the data into the buffer
	if (!is_write)
...
void switch_context(uint32_t *prev_sp, uint32_t *next_sp){
	__asm__ __volatile__(
		...
		// riserviamo lo spazio per 13 registri nello stack 
        ...
        "lw s10, 11 * 4(sp)\n"
        "lw s11, 12 * 4(sp)\n"
        "addi sp, sp, 13 * 4\n"	// eliminiamo lo spazio che abbimo creato prima
